...
{
public:
	HelloTriangle(UINT width, UINT height, wstring name);
	~HelloTriangle();
...
private:
	...
	ComPtr<ID3D12CommandAllocator> m_commandAllocator; // we want enough allocators for each buffer * number of threads (we only have one thread)
